<div id="doenanova-wrap" <?php doenanova_wrap_fadein(); ?>>
	...
	<?php if (is_user_logged_in()) {
		...
		if ($stripe_publishable_key && $stripe_secret_key) { // IF STRIPE KEYS ARE SET BY USER IN WP ADMIN PLUGIN PAGE, THEN SHOW THE DONATION FORM
			...
			if ($customer_id) {
				...
				$latest_charge = \Stripe\Charge::all(array(
					"limit" => 1,
	...
	?>
			...
			********** -->
			...
			<form id="form-doar" action="<?php echo get_admin_url(); ?>admin-post.php" method="POST">
				...
				<!-- ********** AMOUNT ********** -->
				<!-- <div id="amount_wrap">
					<span class="currency"><?php echo doe_na_nova_currency_symbol(); ?></span>
				...
				<?php if ($customer_source) { ?>
					...
					<?php $card = $customer->sources->retrieve($customer_source); ?>
				...
				<?php } else { ?>
					...
					<div class="field">
						...
						<input type="text" class="doenanova-input" id="name" autocomplete="cc-name" required>
					</div>

					<div class="field">
						<label for="card"><?php _e('Card info', 'doenanova'); ?></label>
						<div id="card" class="doenanova-input">
							<!-- Stripe Card Element -->
						</div>
					</div>
					<!-- ********** PAYMENT METHOD - CARD FIELDS ********** -->
