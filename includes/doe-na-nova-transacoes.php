						...
						<?php if (!empty($charges->data)) { ?>
							...
							<?php foreach ($charges->data as $charge) { ?>
								...
								<tr class="<?php echo $status; ?> <?php echo $charge->id ?>">
									<td>
										<div><?php echo $charge->metadata->Purpose; ?></div>
			...
			<?php if (!empty($charges->data)) {
				if ($charges->has_more) { ?>
					...
					<div id="loadmore_recent_transactions-btn_container">
						...
						<!-- Fail message -->
						<div id="loadmore_recent_transactions-fail-message" class="notification is-danger loadmore_recent_transactions-message"><?php _e('Transactions could not be loaded.', 'doenanova'); ?></div>
