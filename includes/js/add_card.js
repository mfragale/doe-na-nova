jQuery(document).ready(function ($) {
	...
	var form = document.getElementById('form-add-card');
	...
	if (mycard) {
		...
		$('#add-card-submit').on('click', function (e) {
			...
			var originalButtonText = $('#add-card-submit').html();

			//Add loading state to the button
			$('#add-card-submit').attr('disabled', 'disabled').addClass('loading').html('<i class="fas fa-circle-notch fa-spin"></i>');
			...
			if ($(form)[0].checkValidity() === false) {
				...
				$('#add-card-submit').removeAttr('disabled').removeClass('loading').html(originalButtonText);
